...
class RowTests: XCTestCase {
    ...
    public func test_get_value() throws {
        ...
        let result = try row.get(SQLiteSwift.Expression<String>("foo"))
    ...
    public func test_get_value_subscript() {
        let row = Row(["\"foo\"": 0], ["value"])
        let result = row[SQLiteSwift.Expression<String>("foo")]
    ...
    public func test_get_type_mismatch_throws_unexpected_null_value() {
        let row = Row(["\"foo\"": 0], ["value"])
        XCTAssertThrowsError(try row.get(SQLiteSwift.Expression<Int>("foo"))) { error in
    ...
    public func test_get_type_mismatch_optional_returns_nil() throws {
        let row = Row(["\"foo\"": 0], ["value"])
        let result = try row.get(SQLiteSwift.Expression<Int?>("foo"))
        XCTAssertNil(result)
    }

    public func test_get_non_existent_column_throws_no_such_column() {
        let row = Row(["\"foo\"": 0], ["value"])
        XCTAssertThrowsError(try row.get(SQLiteSwift.Expression<Int>("bar"))) { error in
    ...
    public func test_get_datatype_throws() {
        ...
        XCTAssertThrowsError(try row.get(SQLiteSwift.Expression<MyType>("foo"))) { error in
